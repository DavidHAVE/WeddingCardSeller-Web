...
$q = mysql_query("select * from gcm_user where registration_id='" . $registration_id . "'") or die(mysql_error()); 
$n = mysql_fetch_row($q); 
    ...
    if ($n > 0) {
        ...
        $sql = "INSERT INTO gcm_user (registration_id) VALUES ('$registration_id')";
        $qur = mysql_query($sql);
        if ($qur) {
            $json = array(
                "status" => 1,
